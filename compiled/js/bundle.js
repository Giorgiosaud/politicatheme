...
(function ($) {
	function obtenerTablaDeDias(m, y) {
		...
		daysPerMonth = [31, y % 4 == 0 && y % 100 != 0 || y % 400 == 0 ? 29 : 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31], tablaDias = '<table><tr>', NumberDayOfWeek = 0;
	...
	function initializer(calendar, date, months, daysShort) {
		...
		prevButton = $('<span />').addClass('calbut prev').html('\&lang\;').appendTo(wrapperHeading), startMonthNumber = date.getMonth(), MonthNumber = ("0" + (startMonthNumber + 1)).slice(-2), startMonthText = months[startMonthNumber], year = date.getFullYear(), dayMonth = $('<span />').html(startMonthText + ' ' + year).addClass('MonthYear').appendTo(wrapperHeading);
		nextButton = $('<span />').addClass('calbut next').html('\&rang\;').appendTo(wrapperHeading), wrapperWeekDays = $("<table />").addClass('weekdays').appendTo(calendar), contenedorDias = $("<div />").addClass('days').appendTo(calendar), diasMesActual = $('<div />').addClass('MesActual').appendTo(contenedorDias), tablaDias = obtenerTablaDeDias(startMonthNumber, year);
		$(tablaDias).appendTo(diasMesActual);
		$('.days').height($('.MesActual').height());
		daysPerMonth = [31, year % 4 == 0 && year % 100 != 0 || year % 400 == 0 ? 29 : 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31], agregarPostAlMes(MonthNumber, year, daysPerMonth[startMonthNumber]);
